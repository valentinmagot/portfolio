    ...
    <section id='about' className={`flex flex-col sm:pt-16 pt-6 w-full`}>
      <div className={`flex-1 ${styles.flexStart} flex-col md:flex-row xl:px-0 sm:px-16 px-6 w-full`}>
        <div className='w-full flex flex-col justify-between items-left py-[6px] py-4 mb-2'>
          <h4 className='font-comfortaa text-[18px] ss:text-[48px] font-bold text-dark'>{t('about.title')}</h4>
          <p className={`${styles.paragraph} mt-8 max-w-[486px] opacity-40`}>
          {t('about.content')}
          </p>
          <a href="mailto:valentin_magot@outlook.com" target='_blank' ><button className='bg-primary mt-4 font-roboto ss:text-[18px] text-[14px] text-bold py-[8px] px-[24px] w-fit rounded-lg hover:animate-pulse'>{t('button.contact')}</button></a>
        </div>
